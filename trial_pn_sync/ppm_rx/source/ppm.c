                ...
                while( delay <= delay_max ){
                    ...
                    for(i=0; i<PN_SEQ_LEN; i+=4){
                        pn_corr += ( (*(pn_seq_buff + i)) > rx_th) * ( (*(ppm_rx + OSF*i)) >rx_th) +
                                   ( (*(pn_seq_buff + i+1)) > rx_th) * ( (*(ppm_rx + OSF*(i+1))) >rx_th) +
                                   ( (*(pn_seq_buff + i+2)) > rx_th) * ( (*(ppm_rx + OSF*(i+2))) >rx_th) +
                                   ( (*(pn_seq_buff + i+3)) > rx_th) * ( (*(ppm_rx + OSF*(i+3))) >rx_th);
                    }
