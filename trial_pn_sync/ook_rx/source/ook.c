        ...
        if (!sync_done){
            ...
            if (delay_max >= 0){
                while( delay <= delay_max ){
                    pn_corr = 0;
                    for(i=0; i<PN_SEQ_LEN; i+=4){
                        pn_corr += ( (*(pn_seq_buff+i)) > rx_th) * ( (*(ook_rx + OSF*i)) >rx_th) +
                       		   ( (*(pn_seq_buff+i+1)) > rx_th) * ( (*(ook_rx + OSF*(i+1))) >rx_th) +
                       		   ( (*(pn_seq_buff+i+2)) > rx_th) * ( (*(ook_rx + OSF*(i+2))) >rx_th) +
                       		   ( (*(pn_seq_buff+i+3)) > rx_th) * ( (*(ook_rx + OSF*(i+3))) >rx_th);
