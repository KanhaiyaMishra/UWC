...
uint32_t ofdm_demod(uint8_t *bin_rx, uint32_t demod_idx, int32_t samp_remng, uint32_t *bits_recvd){
    ...
    static real_t max_of_min = 0.0, auto_corr=0.0, cros_corr=0.0, corr_fact = 0.0, auto_corr_s=0.0, cros_corr_s=0.0;
    ...
    if(sync_done){
        ...
        if (!sync_corrected){
           if ( samp_remng >= SYNC_SYM_LEN ){
                ...
                error = ((sync_err*OSF>0)?0:(sync_err*OSF));
                ...
                #if DEBUG_INFO
                sync_indices[frm_count-1] = demod_idx;
        ...
        if (sync_corrected){
            ...
            if( sym_count==N_SYM && samp_remng >= SYNC_SYM_LEN){
                fprintf(stdout,"RX: Received frame number = %d\n",frm_count++);
                uint32_t new_bits = 0;
                sym_count = 0;
                sync_corrected = 0;
                sync_idx = demod_idx+UNUSED_SAMP;
                samp_remng -= UNUSED_SAMP;
...
//	    rp_AcqGetDataV_WA(ADC_CHANNEL, prev_pos, &samp_recvd, rx_sig_buff, recv_idx, RX_BUFF_SIZE );
    ...
    for(i=0; i<recvd_frms; i++){
        ...
        if ( frm_diff <= 10 && frm_diff >= 1 ){
            ...
            if(error_count[i])
                ...
                fprintf(ber_fp,"RX: Received valid Frame %d with 0 bit errors, Expected Frame = %d, Last Frame=%d\n", rx_frm_num, tx_frm_num, last_rx_frm);
        } else {
            fprintf(ber_fp,"RX: Received invalid Frame %d, Expected Frame = %d, Last FRame = %d. Ignoring for BER Calculation\n", rx_frm_num, tx_frm_num, last_rx_frm);
            missd_frms +=1;
            invalid_frms++;
        }
    ...
    for(i = 0; i <=recvd_frms; i++){
        ...
        fprintf(sync_fp," \t%lf, %lf, %lf, \t%lf\n", dom_tap[0][i], dom_tap[1][i], dom_tap[2][i], est_attn[i]);
    }
