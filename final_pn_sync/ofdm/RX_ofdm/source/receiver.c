...
uint32_t ofdm_demod(uint8_t *bin_rx, uint32_t demod_idx, uint32_t samp_remng, uint32_t *bits_recvd){
    ...
    fprintf(trace_fp,"Sync/Demod Started, Demod Index = %d, Received Samples = %d\n", demod_idx, samp_remng);
            ...
            for( ;samp_remng > OSF*N_FFT; samp_remng-=PRE_DSF) {
                ...
                if( auto_corr > auto_corr_th && corr_fact > 0.7){
                    ...
                    if( window.data[window.front].value > max_of_min ){
                        ...
                        fprintf(trace_fp,"Sync completed, corr_count = %d, remaining samples = %d, SYNC_IDX = %d\n", corr_count, samp_remng, sync_idx);
...
int main(int argc, char** argv){
    ...
    strftime(trace_file, 255,"../log/TRACE_RX_%Y-%m-%d_%H:%M:%S.txt",gmtime(&now));
    strftime(bin_file, 255,"../log/BINARY_RX_%Y-%m-%d_%H:%M:%S.txt",gmtime(&now));
...
//	    rp_AcqGetDataV(RP_CH_2, prev_pos, &samp_recvd, (rx_sig_buff+recv_idx) );
		...
		if ( recv_idx > end_idx ){
            fprintf(trace_fp,"receiver signal buffer filled\n");
			break;
        }
    ...
    fprintf(trace_fp,"receiver signal buffer filled, num of received frames = %d\n", recvd_frms);
