...
const volatile int32_t* gen_GetDataBuffer(rp_channel_t channel)
{
	volatile int32_t* add = NULL;
	if (channel == RP_CH_1)
		add = data_chA;
	else if (channel == RP_CH_2)
		add = data_chB;
...
int gen_GetReadPointer(uint32_t *pos, rp_channel_t channel)
{
    volatile ch_properties_t *properties;
    getChannelPropertiesAddress(&properties, channel);
    *pos = properties->buffReadPointer;
