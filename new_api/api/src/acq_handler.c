...
int acq_GetDataV_WA(rp_channel_t channel,  uint32_t pos, uint32_t* size, float* buffer, uint32_t buff_idx, uint32_t wrap_around)
{
    *size = MIN(*size, ADC_BUFFER_SIZE);
    ...
    for (uint32_t i = 0; i < (*size); ++i) {
        cnts = raw_buffer[(pos + i) % ADC_BUFFER_SIZE];
        buffer[(buff_idx+i)%wrap_around] = cmn_CnvCntToV(ADC_BITS, cnts, gainV, calibScale, dc_offs, 0.0);
    }
