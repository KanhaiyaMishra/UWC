...
int rp_AcqGetDataV_WA(rp_channel_t channel, uint32_t pos, uint32_t* size, float* buffer, uint32_t buff_idx, uint32_t wrap_around)
{
    return acq_GetDataV_WA(channel, pos, size, buffer, buff_idx, wrap_around);
}
...
int rp_GenGetReadPointer(uint32_t *pos, rp_channel_t channel)
{
    return gen_GetReadPointer(pos, channel);
}

volatile int32_t* rp_GenGetAdd(rp_channel_t channel)
{
    return gen_GetDataBuffer(channel);
}

const volatile uint32_t* rp_AcqGetAdd(rp_channel_t channel)
{
    return osc_GetDataBuffer(channel);
}
